...
import { Ticket, Department, ComplaintCategory, EscalationRule, SentEmail, UserSession } from '../types';
...
export default function SLAStatsDashboard({ 
  ...
  const escalationMetrics = useMemo(() => {
    const usersByEmail = new Map(companyUsers.map((user) => [user.email.toLowerCase(), user]));
    return departments.map((department) => {
      const rule = escalationRules.find((item) => item.departmentId === department.id) || null;
      const escalatedTickets = tickets.filter((ticket) => ticket.departmentId === department.id && !!ticket.isEscalated);
      const designationHits: Record<string, number> = {};

      escalatedTickets.forEach((ticket) => {
        const user = ticket.assignedAgentEmail
          ? usersByEmail.get(ticket.assignedAgentEmail.toLowerCase())
          : undefined;
        const designation = user?.designation || (ticket.assignedAgentEmail ? 'Escalated User' : 'Department Head');
        designationHits[designation] = (designationHits[designation] || 0) + 1;
      });

      return {
        departmentId: department.id,
        departmentName: department.name,
        ladder: rule?.designationLevels || [],
        escalatedCount: escalatedTickets.length,
        designationHits: Object.entries(designationHits)
          .sort((a, b) => b[1] - a[1])
          .slice(0, 3)
      };
    });
  }, [companyUsers, departments, escalationRules, tickets]);
      ...
      <div className="bg-white p-4 sm:p-6 rounded-2xl border border-gray-100 shadow-xs mt-6 space-y-6">
        ...
        <div className="rounded-2xl border border-amber-100 bg-amber-50/60 p-4 space-y-3">
          <div className="flex items-center justify-between gap-3">
            <h4 className="text-xs font-bold uppercase tracking-wider text-amber-800">Escalation Metrics Module</h4>
            <span className="rounded-full border border-amber-200 bg-white px-3 py-1 text-[10px] font-bold text-amber-700">
              Department-wise Ladder
            </span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {escalationMetrics.map((item) => (
              <div key={item.departmentId} className="rounded-xl border border-amber-100 bg-white p-3 space-y-2">
                <div className="flex items-start justify-between gap-3">
                  <p className="text-xs font-semibold text-slate-800">{item.departmentName}</p>
                  <span className="text-[10px] font-bold text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full">
                    Escalated: {item.escalatedCount}
                  </span>
                </div>
                <p className="text-[11px] text-slate-600">
                  {item.ladder.length > 0 ? item.ladder.join(' -> ') : 'No ladder configured yet'}
                </p>
                <div className="space-y-1">
                  {item.designationHits.length === 0 ? (
                    <p className="text-[10px] text-slate-400">No escalated designation data yet.</p>
                  ) : (
                    item.designationHits.map(([designation, count]) => (
                      <p key={designation} className="text-[10px] text-slate-500">
                        {designation}: <span className="font-semibold text-slate-700">{count}</span>
                      </p>
                    ))
                  )}
                </div>
