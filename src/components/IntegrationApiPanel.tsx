...
export default function IntegrationApiPanel({ token }: IntegrationApiPanelProps) {
  ...
  return <div className="integration-api-panel grid grid-cols-1 gap-4 xl:grid-cols-2">
    ...
    <section className="order-first xl:col-span-2 rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_18px_44px_rgba(15,23,42,0.06)] space-y-4">
      ...
      {!zoho ? <p className="text-xs text-slate-400">Loading Zoho Desk configuration…</p> : <>
        <div className="grid gap-3 md:grid-cols-2 text-xs"><div className={`rounded-xl border p-3 ${zoho.configured ? 'border-emerald-100 bg-emerald-50 text-emerald-800' : 'border-amber-100 bg-amber-50 text-amber-800'}`}><b>Webhook verification</b><p className="mt-1">{zoho.configured ? 'Organization ID and secure callback token are configured.' : 'Add Zoho environment variables on Render.'}</p></div><div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-slate-700"><b>Recent events</b><p className="mt-1">Created: {zoho.statistics?.PROCESSED || 0} · Duplicates: {zoho.statistics?.DUPLICATE || 0} · Failed: {zoho.statistics?.FAILED || 0}</p></div></div>
